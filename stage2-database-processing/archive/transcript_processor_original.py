...
def create_secondary_sections(primary_content: str, primary_type: str, client: OpenAI) -> List[Dict]:
    ...
    # Handle large content by chunking instead of truncating
    max_chars = 15000  # Leave room for prompt
    
    if len(primary_content) <= max_chars:
        # Content fits in one pass
        content_chunks = [primary_content]
    else:
        # Split into overlapping chunks to avoid losing content
        content_chunks = []
        chunk_size = max_chars
        overlap = 1000  # Overlap to maintain context
        
        start = 0
        while start < len(primary_content):
            end = start + chunk_size
            chunk = primary_content[start:end]
            
            # Try to end at sentence boundary
            if end < len(primary_content):
            ...
            start = end - overlap
            if start >= len(primary_content):
                break
        
        if DEBUG_MODE:
            logger.warning(f"⚠ Large {primary_type} section ({len(primary_content)} chars) split into {len(content_chunks)} chunks for processing")
